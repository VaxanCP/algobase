...
/**
 * \brief Link Cut Tree implementation for subtree nodes update, and
 * subtree query.
 *
 * \tparam Group
 * \tparam Gact
 * \tparam Map
 * \tparam Comp Invertible Commutative Binary Operation
 * \tparam CompId
 * \tparam CompInv
 * \tparam BinOp Invertible Commutative Binary Operation(For path query,
 * invertiblity doesn't matter).
 * \tparam BinOpId
 * \tparam BinOpInv
 */
...
class subtree_aggregation_handler
    : public detail::basic_lct<
          ...
          impl::subtree_aggregation_node<Group, Gact>> {
  using self_type = subtree_aggregation_handler;
  using node_type = impl::subtree_aggregation_node<Group, Gact>;
...
public:
  ...
  subtree_aggregation_handler(int n, Map map, Comp comp, CompId comp_id,
                              CompInv comp_inv, BinOp binop, BinOpId binop_id,
                              BinOpInv binop_inv) noexcept
      : base_type{n, {binop_id(), binop_id(), binop_id(), comp_id()}},
        map_{std::move(map)},
        comp_{std::move(comp)},
  ...
  subtree_aggregation_handler(int n, const Group& def, Map map, Comp comp,
                              CompId comp_id, CompInv comp_inv, BinOp binop,
                              BinOpId binop_id, BinOpInv binop_inv) noexcept
      : base_type{n, {def, def, binop_id(), comp_id()}},
        map_{std::move(map)},
        comp_{std::move(comp)},
        comp_id_{std::move(comp_id)},
        comp_inv_{std::move(comp_inv)},
